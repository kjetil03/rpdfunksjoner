...
#' @param hrcutfilter numeric. Angi i prosentpoeng hvor store avvik i haircuts fra gjennomsnittet man
...
#' }
#'
plot_repo_haircuts <- function(df,
  ...
  df = df %>% filter(ReportingAgentName %in% reporting,
                     ...
                     TxTp %in% direction,
                     Hrcut > mean(df$Hrcut) - hrcutfilter & Hrcut < mean(df$Hrcut) + hrcutfilter)
  ...
  ymin = min(0, min(df$Hrcut)-0.5)
  ymax = max(df$Hrcut)+0.5
  ...
  if(all(direction == "LEND")) {
    dirtext = "Haircut i utlån av kroner i repo mot"
  }

  if(all(direction == "BORR")) {
    dirtext = "Haircut i innlån av kroner i repo mot"
  }

  if(length(direction) == 2) {
    dirtext = "Haircut i repo mot"
  }
