...
find_tenors <- function(df, tradedate = "TradDt",
  ...
  df = df %>%
    ...
    mutate(cal_days_to_settle = as.double(SttlmDt - TradDt),
           ...
           tenor = if_else( (biz_days_to_settle <=5)  & (cal_maturity_days == 7) | (biz_maturity_days == 5), "1w", tenor),
           tenor = if_else((cal_maturity_days == 14) | (biz_maturity_days == 10), "2w", tenor),
           tenor = if_else((cal_maturity_days == 21) | (biz_maturity_days == 15), "3w", tenor),
           tenor = if_else(cal_maturity_days %in% seq(25,35, by = 1), "1m", tenor),
           tenor = if_else(cal_maturity_days %in% seq(55, 65, by = 1), "2m", tenor),
           tenor = if_else(cal_maturity_days %in% seq(80, 100, by = 1), "3m", tenor),
           tenor = if_else(cal_maturity_days %in% seq(110, 130, by = 1), "4m", tenor),
           tenor = if_else(cal_maturity_days %in% seq(140, 160, by = 1), "5m", tenor),
           tenor = if_else(cal_maturity_days %in% seq(170, 190, by =1), "6m", tenor),
           tenor = if_else(cal_maturity_days > 360, "1y", tenor),
