...
#' repos <- repo()
#' ## Hent ut kun omsetning mellom bankene som rapporterer, siden 30. juni 2020,
#' ## og nett transaksjoner mellom dem:
#' repos <- repo(start_date = "2020-06-30",
#' counterparties = "reportingagents", netting = TRUE)

repo <- function(start_date = "2019-11-30",
         counterparties = "all",
         netting = FALSE,
         ctrInfo = TRUE,
         additional_fields = c()) {
  ...
  sql_srch = "SELECT a.TransKey, CAST(TradDt as FLOAT) AS TradDt, CAST([SttlmDt] as FLOAT) as SttlmDt,
           CAST(MtrtyDt AS FLOAT) AS MtrtyDt, [TxNmnlAmt], [TxTp], RptgAgt, CtrPtyLEI,
		      [DealRate], [Hrcut], RateTp

		      ---, [SpclCollInd], StatusOnTransaction, b.ISIN, b.NmnlAmt

          --- __CtrPtyInfo__
          ---__additional_fields__

          FROM [NBIP].[NBIP].[RealTransactions] as a
          ---left join [NBIP].[NBIP].[Collateral] as b on a.TransKey = b.TransKey
  ...
  #### Gammel metode, kun her intil videre ######
  ...
  collateral_srch <- "SELECT TransKey, ISIN, NmnlAmt, PoolSts, Sctr
                      FROM [NBIP].[NBIP].[Collateral]"

  collateral_list <- sqlQuery(dbhandle, collateral_srch)

  raw_data = left_join(raw_data, collateral_list, by = "TransKey")
  ...
  if(counterparties %in% c("all", "reportingagents")) {
    if(netting) {
      ...
      raw_data = raw_data %>% filter(!(CtrPtyLEI %in% rpt_agents$ReportingAgentLEI & TxTp == "LEND"))
  ...
  repo = raw_data %>% find_tenors(settlementdate = "SttlmDt")
  repo = raw_data %>% find_tenors_2(settlementdate = "SttlmDt")
  # Hent info om isin fra Nordic Bond Pricing-databasen i NBDataHub
  conn = 'driver={SQL Server};server=wm-x-s-31;database = NBDataHub;trusted_connection=true'

  sql_isin = "SELECT  [ISIN]
      ,[ShortName]
      ,[IssueDate]
      ,[CurrentOutstandingAmount]
      ,[MaximumOutstandingAmount]
      ,[FaceValue]
      ,[CurrentMargin]
      ,[CurrentCouponRate]
      ,[ReferenceRate]
      ...
      ,[Issuer_Name]
      ,[MaturityDate]
      ,[CapitalWeight_20120319]
      ,[Issuer_IndustryGroup]
      ,[CapitalWeight]
  FROM [NBDataHub].[NordicTrustee].[Issue] where MaturityDate >= '2019-11-01'"


  dbhandle <- odbcDriverConnect(conn)
  isin_info <- sqlQuery(dbhandle,
                       sql_isin) %>% mutate(ISIN = as.character(ISIN))

  close(dbhandle)

  repo = repo %>% mutate(ISIN = as.character(ISIN)) %>%  left_join(., isin_info, by = "ISIN")

  repo = repo %>% mutate(Issuer_Business = as.character(Issuer_Business),
                         Issuer_Business = if_else(is.na(Issuer_Business), "Other", Issuer_Business),
                         collateral_category = "Annet",
                         collateral_category = if_else(Issuer_Business %in% c("KRA", "KRF"), "OMF", collateral_category),
                         collateral_category = if_else(Issuer_Business %in% c("KOM", "FYL"), "Kommune og fylkeskommune", collateral_category),
                         collateral_category = if_else(Issuer_Business %in% "STA" , "Stat", collateral_category),
                         collateral_category = if_else(Issuer_IndustryGroup %in% 20 , "Vei og bompenger", collateral_category),
                         collateral_category = if_else(Issuer_Business %in% c("FOB", "SPB"), "Senior Bank", collateral_category),
                         collateral_category = if_else(PoolSts %in% "POOL", "Collateral pool", collateral_category))

  #Rydd litt i navn.
  repo$ReportingAgentName = str_replace(repo$ReportingAgentName, "\\s+$", "")
  repo = repo %>% mutate(CtrPtyLEI = as.character(CtrPtyLEI),
  ...
  repo = repo %>% left_join(., rpt_agents, by = "CtrPtyLEI") %>%
    mutate(CounterPartyName = if_else(is.na(Name), CounterPartyName, Name)) %>%
    select(-Name)

  repo$CounterPartyName = str_replace(repo$CounterPartyName, "\\s+$", "")
